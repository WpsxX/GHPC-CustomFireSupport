...
| `SlotN_Type`                  | 字符串 | 槽 1 = `Artillery`；槽 2 = `ArtillerySmoke`；槽 3 = `ArtilleryIllumination`；槽 4/5/6 = `CASSupport`                     | 见 §4.1                         | 槽位类型。                                                                                                         |
...
| `SlotN_Missions`              | 整数  | 槽 1/2/3 = `3`；槽 4/5/6 = `2`                                                                                       | `-1`，或 `0` ~ `999`             | 可用**呼叫次数**。`-1` = 无限。`0` = 无法点击。                                                                              |
| `SlotN_RoundsPerCall`         | 整数  | `-1`                                                                                                              | `-1`（或任何小于 1 的值），或 `1` ~ `999` | **单次呼叫的弹数**。`-1` = 沿用游戏炮组自己的弹数；`>0` = 强制指定。                                                                   |
| `SlotN_Munition`              | 字符串 | 槽 2 = `Smoke`；槽 3 = `Illumination`；其余 = `AntiPersonnel`                                                           | 见 §4.2；留空 = 跟随 `Type`          | **弹种**。与 `Type` 不匹配时仍会生效。                                                                                     |
| `SlotN_Weapon`                | 字符串 | `Any`                                                                                                             | 见 §4.3                         | 影响面板上显示的武器类别（`WeaponType`）。                                                                                   |
| `SlotN_ImpactDelaySeconds`    | 小数  | `1.0`                                                                                                             | `-1` ~ `100`                   | **首弹延迟倍率**`1` = 游戏原值；`0`~`1` 之间 = 取消延迟、首弹立即到；`0` / 负数 = **一帧打完**。**不影响发间隔**。                                  |
| `SlotN_InterShotDelaySeconds` | 小数  | `1.0`                                                                                                             | `-1` ~ `100`                   | **发间隔倍率**。`1` = 原值；`0.5` = 一半；`0` = 一帧打完。**不影响首弹延迟**。                                                         |
...
| `SlotN_AmmoName`              | 字符串 | `""`                                                                                                              | 任意文本；留空 = 不筛选                  | 可选筛选用：**弹药名包含该文本**才会被选中，例如 `"155mm"`。留空 = 取该弹种排序最优的模板。                                                        |
| `SlotN_CasFlyover`            | 字符串 | `""`                                                                                                              | `""` / `SinglePass` / `Linger` | CAS **飞行方式**。留空 = 按机型自动；`SinglePass` = 单次通过；`Linger` = 盘旋。仅 CAS 槽使用。                                          |
