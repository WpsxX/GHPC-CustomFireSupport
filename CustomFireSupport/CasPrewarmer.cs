...
namespace CustomFireSupport
{
    ...
    internal static class CasPrewarmer
    {
        ...
        // The bundle carries CAS hardpoints whose AmmoType.CachedIndex was generated by the export,
        // not by this game session. Keep exact AmmoType references so the impact-cache patch can repair
        // only the mod's imported rounds and leave unrelated vanilla ammunition untouched.
        private static readonly HashSet<AmmoType> _bundleAmmo =
            new HashSet<AmmoType>(AmmoReferenceComparer.Instance);
        ...
        private static void LoadExtraBundle()
        {
            ...
            try
            {
                ...
                // LoadAllAssets returns the hardpoint prefab, while the projectile visual and AmmoType
                // may be an indirect dependency. Record the public ammo reference from every bundled
                // hardpoint so its effect/decal cache can be rebuilt when the first round impacts.
                for (int i = 0; i < _bundlePrefabs.Count; i++)
                {
                    CASHardpoint[] mountedHardpoints = _bundlePrefabs[i].GetComponentsInChildren<CASHardpoint>(true);
                    for (int j = 0; j < mountedHardpoints.Length; j++)
                    {
                        try
                        {
                            AmmoType ammo = mountedHardpoints[j] == null ? null : mountedHardpoints[j].Ammo;
                            if (ammo != null)
                            {
                                _bundleAmmo.Add(ammo);
                                // ShotVisual is commonly an indirect bundle dependency and is not
                                // returned by LoadAllAssets. Include it in the material repair walk.
                                GameObject visual = ammo.ShotVisual;
                                if (visual != null && !_bundleObjects.Contains(visual))
                                {
                        ...
                        {
                            Log.Verbose("CAS pre-warm: could not read ammo from bundled hardpoint '" +
                                (mountedHardpoints[j] == null ? "<null>" : mountedHardpoints[j].name) + "' (" +
                                ex.GetType().Name + ").");
                        }
                    }
                }
